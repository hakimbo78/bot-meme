    ...
    def _log_debug(self, candidate):
        ...
        if candidate['verdict'] == 'SKIP':
            ...
            fmt_bd = (f"{{liq:{bd['liquidity']}, buy:{bd['buy_velocity']}, "
                      ...
                      f"age:{bd['age_bonus']}, risk:{bd['risk_penalty']}}}")
            
            skip = candidate['skip_reason']
            print(f"[SOLANA][DEBUG] {symbol} | Score={score} | Breakdown={fmt_bd} SkipReason={skip}", flush=True)
            
        else:
            # Pass Format
            # [SOLANA][DEBUG] SYMBOL | Score=72 | QUALIFIED_FOR=WATCH
            verdict = candidate['verdict']
            print(f"[SOLANA][DEBUG] {symbol} | Score={score} | QUALIFIED_FOR={verdict}", flush=True)
